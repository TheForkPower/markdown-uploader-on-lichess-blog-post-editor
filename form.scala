      ...
      post.toOption match
        ...
        case _ =>
          div(
            ...
            div(cls := "form-group form-full")(
  form3.file.markdown("markdownFile"),
  label(forId := "markdownFile")(trans.ublog.uploadMarkdownFile())
)

val markdownFile = request.body.file("markdownFile")
markdownFile.map { file =>
  val fileName = file.filename
  val fileExtension = fileName.substring(fileName.lastIndexOf('.') + 1).toLowerCase()
  
  if (fileExtension == "md") {
    val markdownContent = scala.io.Source.fromFile(file.ref.path.toFile).mkString
    ...
    val popupScript = "alert('Invalid file format. Only .md files are supported.');"
  }
}
            form3.split(
              form3.checkbox(
